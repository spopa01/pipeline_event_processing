...
std::vector<int> run_big_events_test( event_processor_pipeline_sptr_t const& pipeline, int num_events, int num_samples_per_event ){
	std::vector<int> computed_values; //move semantics...
...
void run_big_events_test(){
  auto num_live_tokens = 16;
	auto num_events = 32;
  auto num_samples_per_event = 20000000;
  ...
  (*tbb_pipeline)( std::make_shared<start_event_t>() );
	...
	//PROCESS USING PIPELINES
  auto simple_computed_values = run_big_events_test( simple_pipeline, num_events, num_samples_per_event );
	auto tbb_computed_values = run_big_events_test( tbb_pipeline, num_events, num_samples_per_event );
  ...
  //CHECK THE RESULTS
  std::cout << "BIG_EVENTS_TEST ... " << ( simple_computed_values.size() > 0 && simple_computed_values == tbb_computed_values ? "PASSED" : "FAILED" ) << std::endl;
